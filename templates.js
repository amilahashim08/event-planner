class TemplateManager {
  ...
  setupEventListeners() {
    ...
    const createDesignTemplateBtn = document.getElementById(
      "createDesignTemplateBtn"
    );
    ...
    const createBudgetTemplateBtn = document.getElementById(
      "createBudgetTemplateBtn"
    );
    ...
    const saveBudgetDetailsBtn = document.getElementById(
      "saveBudgetDetailsBtn"
    );
    ...
    const questionnaireSearchInput = document.getElementById(
      "questionnaireTemplateSearch"
    );
    ...
    const questionnaireEventFilter = document.getElementById(
      "questionnaireEventFilter"
    );
    ...
    const createQuestionnaireTemplateBtn = document.getElementById(
      "createQuestionnaireTemplateBtn"
    );
    ...
    const questionnaireEventTypeDropdown = document.getElementById(
      "questionnaireEventTypeDropdown"
    );
    if (questionnaireEventTypeDropdown) {
      questionnaireEventTypeDropdown.addEventListener("click", (e) => {
        if (e.target.classList.contains("dropdown-item")) {
          ...
          // Update active state
          document
            .querySelectorAll("#questionnaireEventTypeDropdown .dropdown-item")
            .forEach((item) => {
    ...
    const createProposalsTemplateBtn = document.getElementById(
      "createProposalsTemplateBtn"
    );
    ...
    const createContractsTemplateBtn = document.getElementById(
      "createContractsTemplateBtn"
    );
    ...
    const createQuotesTemplateBtn = document.getElementById(
      "createQuotesTemplateBtn"
    );
    ...
    const createInvoicesTemplateBtn = document.getElementById(
      "createInvoicesTemplateBtn"
    );
    ...
    const createPaymentsTemplateBtn = document.getElementById(
      "createPaymentsTemplateBtn"
    );
  ...
  setupDropdownSelection() {
    ...
    const dropdownItems = document.querySelectorAll(
      "#eventTypeDropdown .dropdown-item"
    );
    ...
    const defaultItem = document.querySelector(
      "#eventTypeDropdown .dropdown-item.active"
    );
    ...
    const designDropdownItems = document.querySelectorAll(
      "#designEventTypeDropdown .dropdown-item"
    );
    ...
    const designDefaultItem = document.querySelector(
      "#designEventTypeDropdown .dropdown-item.active"
    );
    ...
    const budgetDropdownItems = document.querySelectorAll(
      "#budgetEventTypeDropdown .dropdown-item"
    );
    ...
    const budgetDefaultItem = document.querySelector(
      "#budgetEventTypeDropdown .dropdown-item.active"
    );
    ...
    const proposalsDropdownItems = document.querySelectorAll(
      "#proposalsEventTypeDropdown .dropdown-item"
    );
    const proposalsEventTypeInput =
      document.getElementById("proposalsEventType");
    ...
    const proposalsDefaultItem = document.querySelector(
      "#proposalsEventTypeDropdown .dropdown-item.active"
    );
    if (proposalsDefaultItem && proposalsEventTypeInput) {
      proposalsEventTypeInput.value = proposalsDefaultItem.textContent;
      proposalsEventTypeInput.dataset.value =
        proposalsDefaultItem.dataset.value;
    }

    // Contracts template modal dropdown
    const contractsDropdownItems = document.querySelectorAll(
      "#contractsEventTypeDropdown .dropdown-item"
    );
    const contractsEventTypeInput =
      document.getElementById("contractsEventType");
    ...
    const contractsDefaultItem = document.querySelector(
      "#contractsEventTypeDropdown .dropdown-item.active"
    );
    if (contractsDefaultItem && contractsEventTypeInput) {
      contractsEventTypeInput.value = contractsDefaultItem.textContent;
      contractsEventTypeInput.dataset.value =
        contractsDefaultItem.dataset.value;
    }

    // Quotes template modal dropdown
    const quotesDropdownItems = document.querySelectorAll(
      "#quotesEventTypeDropdown .dropdown-item"
    );
    ...
    const quotesDefaultItem = document.querySelector(
      "#quotesEventTypeDropdown .dropdown-item.active"
    );
    ...
    const paymentsDropdownItems = document.querySelectorAll(
      "#paymentsEventTypeDropdown .dropdown-item"
    );
    ...
    const paymentsDefaultItem = document.querySelector(
      "#paymentsEventTypeDropdown .dropdown-item.active"
    );
                ...
                <div class="card-content">
                    ...
                    <div>
                        ...
                        <button class="questionnaire-action-btn" onclick="event.stopPropagation(); templateManager.editQuestionnaireTemplate(${template.id})" title="Edit">
                            <i class="fas fa-edit" style="color:"#7DB477"></i>
                        </button>
                           ...
                           <button class="budget-action-btn delete" onclick="event.stopPropagation(); templateManager.deleteBudgetTemplate(${template.id})" title="Delete">
                                <i class="fas fa-trash" style="color:"#DF464B"></i>
                            </button>
                    </div>
                    </div>
  ...
  handleDesignSearch(searchTerm) {
    const filtered = this.designTemplates.filter(
      (template) =>
        template.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
        template.category.toLowerCase().includes(searchTerm.toLowerCase())
    );
  ...
  handleDesignFilter(eventType) {
    if (eventType) {
      this.filteredDesignTemplates = this.designTemplates.filter(
        (template) => template.type === eventType
      );
    } else {
      this.filteredDesignTemplates = [...this.designTemplates];
  ...
  renderDesignTemplates() {
    ...
    const templatesHTML = this.filteredDesignTemplates
      .map(
        (template) => `
  ...
  handleCreateDesignTemplate() {
    ...
    // Close modal
    const modal = bootstrap.Modal.getInstance(
      document.getElementById("createDesignTemplateModal")
    );
    modal.hide();
  ...
  deleteDesignTemplate(templateId) {
    if (confirm("Are you sure you want to delete this design template?")) {
      this.designTemplates = this.designTemplates.filter(
        (t) => t.id !== templateId
      );
      this.filteredDesignTemplates = this.filteredDesignTemplates.filter(
        (t) => t.id !== templateId
      );
      this.renderDesignTemplates();
  ...
  handleBudgetSearch(searchTerm) {
    const filtered = this.budgetTemplates.filter(
      (template) =>
        template.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
        template.category.toLowerCase().includes(searchTerm.toLowerCase())
    );
  ...
  handleBudgetFilter(eventType) {
    if (eventType) {
      this.filteredBudgetTemplates = this.budgetTemplates.filter(
        (template) => template.type === eventType
      );
    } else {
      this.filteredBudgetTemplates = [...this.budgetTemplates];
  ...
  renderBudgetTemplates() {
    ...
    const templatesHTML = this.filteredBudgetTemplates
      .map(
        (template) => `
  ...
  handleCreateBudgetTemplate() {
    ...
    // Close modal
    const modal = bootstrap.Modal.getInstance(
      document.getElementById("createBudgetTemplateModal")
    );
    modal.hide();
  ...
  deleteBudgetTemplate(templateId) {
    if (confirm("Are you sure you want to delete this budget template?")) {
      this.budgetTemplates = this.budgetTemplates.filter(
        (t) => t.id !== templateId
      );
      this.filteredBudgetTemplates = this.filteredBudgetTemplates.filter(
        (t) => t.id !== templateId
      );
      this.renderBudgetTemplates();
  ...
  handleQuestionnaireSearch(searchTerm) {
    this.filteredQuestionnaireTemplates = this.questionnaireTemplates.filter(
      (template) =>
        template.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  renderQuestionnaireTemplates() {
    ...
    grid.innerHTML = this.filteredQuestionnaireTemplates
      .map(
        (template) => `
  ...
  handleCreateQuestionnaireTemplate() {
    ...
    // Close modal
    const modal = bootstrap.Modal.getInstance(
      document.getElementById("createQuestionnaireTemplateModal")
    );
    modal.hide();
  ...
  deleteQuestionnaireTemplate(templateId) {
    if (
      ...
      this.questionnaireTemplates = this.questionnaireTemplates.filter(
        (t) => t.id !== templateId
      );
      this.filteredQuestionnaireTemplates =
        this.filteredQuestionnaireTemplates.filter((t) => t.id !== templateId);
      this.renderQuestionnaireTemplates();
      this.showMessage(
  ...
  renderProposalsTemplates() {
    ...
    grid.innerHTML = this.filteredProposals
      .map(
        (proposal, index) => `
            <div class="template-card" onclick="templateManager.viewProposal(${
              proposal.id
            })">
                <div class="template-card-image" style="background-image: url('${this.getTemplateImage(
                  index
                )}')">
                ...
                <div class="template-card-content">
                    <div class="template-card-info">
                        <div class="template-card-left">
                            ...
                            <p class="template-date">Updated On: ${
  ...
  handleCreateProposalsTemplate() {
    ...
    // Close modal
    const modal = bootstrap.Modal.getInstance(
      document.getElementById("createProposalsTemplateModal")
    );
    modal.hide();
  ...
  renderContractsTemplates() {
    ...
    grid.innerHTML = this.filteredContracts
      .map(
        (contract, index) => `
            <div class="template-card" onclick="templateManager.viewContract(${
              contract.id
            })">
                <div class="template-card-image" style="background-image: url('${this.getTemplateImage(
                  index
  ...
  handleCreateContractsTemplate() {
    ...
    // Close modal
    const modal = bootstrap.Modal.getInstance(
      document.getElementById("createContractsTemplateModal")
    );
    modal.hide();
  ...
  viewContract(contractId) {
    ...
    document.getElementById("contractDetailTitle").textContent = contract
      ? contract.title
      : `Contract ${contractId}`;
  ...
  renderQuotesTemplates() {
    ...
    grid.innerHTML = this.filteredQuotes
      .map(
        (quote, index) => `
            <div class="template-card" onclick="templateManager.viewQuote(${
              quote.id
            })">
                <div class="template-card-image" style="background-image: url('${this.getTemplateImage(
                  index
  ...
  handleCreateQuotesTemplate() {
    ...
    // Close modal
    const modal = bootstrap.Modal.getInstance(
      document.getElementById("createQuotesTemplateModal")
    );
    modal.hide();
  ...
  viewQuote(quoteId) {
    ...
    document.getElementById("quoteDetailTitle").textContent = quote
      ? quote.title
      : `Quote ${quoteId}`;
  ...
  renderInvoicesTemplates() {
    ...
    grid.innerHTML = this.filteredInvoices
      .map(
        (invoice, index) => `
            <div class="template-card" onclick="templateManager.viewInvoice(${
              invoice.id
            })">
                <div class="template-card-image" style="background-image: url('${this.getTemplateImage(
                  index
  ...
  handleCreateInvoicesTemplate() {
    ...
    // Close modal
    const modal = bootstrap.Modal.getInstance(
      document.getElementById("createInvoicesTemplateModal")
    );
    modal.hide();
  ...
  viewInvoice(invoiceId) {
    ...
    document.getElementById("invoiceDetailTitle").textContent = invoice
      ? invoice.title
      : `Invoice ${invoiceId}`;
  ...
  renderPaymentsTemplates() {
    ...
    grid.innerHTML = this.filteredPayments
      .map(
        (payment, index) => `
            <div class="template-card" onclick="templateManager.viewPayment(${
              payment.id
            })">
                <div class="template-card-image" style="background-image: url('${this.getTemplateImage(
                  index
  ...
  handleCreatePaymentsTemplate() {
    ...
    // Close modal
    const modal = bootstrap.Modal.getInstance(
      document.getElementById("createPaymentsTemplateModal")
    );
    modal.hide();
  ...
  viewPayment(paymentId) {
    ...
    document.getElementById("paymentDetailTitle").textContent = payment
      ? payment.title
      : `Payment ${paymentId}`;
  ...
  loadPaymentDetailData(paymentId) {
    ...
    if (payment) {
      ...
      document.getElementById("paymentId").value =
        payment.paymentId || `PAY-${paymentId.toString().padStart(4, "0")}`;
      ...
      document.getElementById("paymentClientName").value =
        payment.clientName || "";
      document.getElementById("paymentClientEmail").value =
        payment.clientEmail || "";
      ...
      document.getElementById("gatewayResponse").value =
  ...
  setActiveNavItem() {
    ...
    const templatesNav = document.querySelector(
      '.sidebar .nav-item a[href="templates.html"]'
    );
    if (templatesNav) {
